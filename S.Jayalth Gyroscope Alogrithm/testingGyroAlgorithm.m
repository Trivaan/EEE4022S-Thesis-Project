...
for i = 1:length(rawData.fileName)
    [allData]= testingGyroscopeAlgorithm(rawData.fileName(i),rawData.lowerLimit(i), rawData.upperLimit(i));
    [gyroSteps] = SampathJayalath(allData.GyroX,allData.GyroY,allData.GyroZ,rawData.upperLimit(i)-rawData.lowerLimit(i));
end

%% Function that sorts the data into the pre-processing stage and subbsequent filtering stage
function [allData] = testingGyroscopeAlgorithm(fileName, lowerLimit, upperLimit)
